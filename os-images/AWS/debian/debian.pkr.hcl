...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "apt-get install -y bash git vim openssh-server curl tar xz-utils unzip gnupg"
    ]
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      if [ '${var.distro_arch}' = 'x86_64' ]; then
        ...
        curl -f https://s3.amazonaws.com/amazoncloudwatch-agent/debian/${var.distro_arch == "x86_64" ? "amd64" : "arm64"}/latest/amazon-cloudwatch-agent.deb -o /tmp/amazon-cloudwatch-agent.deb
        curl -f https://s3.amazonaws.com/amazoncloudwatch-agent/debian/${var.distro_arch == "x86_64" ? "amd64" : "arm64"}/latest/amazon-cloudwatch-agent.deb.sig -o /tmp/amazon-cloudwatch-agent.deb.sig
  ...
  post-processor "manifest" {
    custom_data = {
      ...
      instance_type              = var.instance_type
      is_windows                 = false
      cloudwatch-agent-available = var.distro_arch == "x86_64" ? true : false
