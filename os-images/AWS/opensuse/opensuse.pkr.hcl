...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "zypper --non-interactive install --auto-agree-with-licenses dbus-1 systemd git vim sudo curl openssh tar unzip"
    ]
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "curl -f https://s3.amazonaws.com/amazoncloudwatch-agent/suse/${var.distro_arch == "x86_64" ? "amd64" : "arm64"}/latest/amazon-cloudwatch-agent.rpm -o /tmp/amazon-cloudwatch-agent.rpm",
      "curl -f https://s3.amazonaws.com/amazoncloudwatch-agent/suse/${var.distro_arch == "x86_64" ? "amd64" : "arm64"}/latest/amazon-cloudwatch-agent.rpm.sig -o /tmp/amazon-cloudwatch-agent.rpm.sig",
