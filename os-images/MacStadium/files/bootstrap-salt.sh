...
install_macosx_git_deps() {
    ...
    # Install PIP
    $_PYEXE /tmp/get-pip.py && rm -f /tmp/get-pip.py || return 1

    __git_clone_and_checkout || return 1

    __PIP_REQUIREMENTS="pkg/osx/req.txt"
    requirements_file="${_SALT_GIT_CHECKOUT_DIR}/${__PIP_REQUIREMENTS}"
...
install_macosx_git() {
    ...
    if [ -f "${_SALT_GIT_CHECKOUT_DIR}/salt/syspaths.py" ]; then
        USE_SETUPTOOLS=1 $_PYEXE setup.py --salt-config-dir="$_SALT_ETC_DIR" --salt-cache-dir="${_SALT_CACHE_DIR}" ${SETUP_PY_INSTALL_ARGS} install --prefix=/opt/salt || return 1
    else
        USE_SETUPTOOLS=1 $_PYEXE setup.py ${SETUP_PY_INSTALL_ARGS} install --prefix=/opt/salt || return 1
    fi
