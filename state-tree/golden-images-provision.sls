...
include:
  ...
  - lxc
  {%- endif %}
  {%- if (grains['os'] not in ['Amazon', 'FreeBSD', 'Debian', 'Ubuntu', 'Windows'] and grains['os_family'] != 'Suse' and not grains['osrelease'].startswith('5.')) or (grains['os'] == 'Ubuntu' and grains['osrelease'].startswith('14.')) %}
  - npm
  - bower
  {%- endif %}
  {%- if grains['os'] == 'Fedora' %}
