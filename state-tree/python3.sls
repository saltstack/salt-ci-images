...
python3:
  file.managed:
    - source: https://www.python.org/ftp/python/3.8.10/python-3.8.10-macosx10.9.pkg
    - name: /tmp/python-3.8.pkg
    - user: vagrant
    - group: wheel
    - skip_verify: True
    - onlyif: '[ ! -d /Library/Frameworks/Python.framework/Versions/3.8 ]'
...
install-certs-py3:
  cmd.run:
    - name: /Applications/Python\ 3.8/Install\ Certificates.command
    - runas: vagrant
...
add-python3-to-path:
  file.append:
    - names:
      - /etc/paths.d/python:
        - text: '/Library/Frameworks/Python.framework/Versions/3.8/bin'
  environ.setenv:
    - name: PATH
    - value: '/Library/Frameworks/Python.framework/Versions/3.8/bin:{{ salt.cmd.run_stdout('echo $PATH', python_shell=True).strip() }}'
    - update_minion: True
