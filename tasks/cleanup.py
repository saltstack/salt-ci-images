...
def cleanup_aws(
    ...
    if HAS_BOTO is False:
        exit_invoke(
            1,
            "Please install boto3: 'pip install -r {}'".format(
    ...
    if HAS_BLESSINGS is False:
        exit_invoke(
            1,
            "Please install blessings: 'pip install -r {}'".format(
                os.path.join(REPO_ROOT, "requirements", "base.txt")
            ),
        )
    ...
    if name_filter is None:
        ...
        if staging is True:
            name_filter += "-staging"
        name_filter += f"/{distro.lower()}"
        if distro_version:
            name_filter += f"/{distro_version}"
    ...
    filters = [
        ...
        {"Name": "tag:Promoted", "Values": ["1" if promoted else "0"]},
    ]
    response = client.describe_images(Filters=filters)
    if response["ResponseMetadata"]["HTTPStatusCode"] != 200:
        exit_invoke(1, f"Failed to get images. Full response:\n{pprint.pformat(response)}")
    ...
    for image_details in images_to_delete:
        ...
        for block_device in block_devices:
            ...
            if "Ebs" not in block_device:
                warn(
                    "Skipping non EBS block device with details:\n{}",
                    pprint.pformat(block_device),
                    5 * " ",
                )
                continue
