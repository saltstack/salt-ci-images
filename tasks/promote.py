...
def promote_container(ctx,
    ...
    if not container:
        exit_invoke(1, 'You need to provide --container')

    if username is None and 'DOCKER_USERNAME' in os.environ:
        username = os.environ['DOCKER_USERNAME']

    if password is None and 'DOCKER_PASSWORD' in os.environ:
        password = os.environ['DOCKER_PASSWORD']

    promoted_container = container.replace('ci-staging', 'ci')

    info('Promoting {} to {}', container, promoted_container)

    if dry_run:
        exit_invoke(0, 'The container {} would have been promoted to {}', container, promoted_container)
    ...
    ctx.run('docker pull {}'.format(container), echo=True)
    ctx.run('docker tag {} {}'.format(container, promoted_container), echo=True)
    if username and password:
        ctx.run('docker login --username {} --password {}'.format(username, password))
    ctx.run('docker push {}'.format(promoted_container), echo=True)
    if username and password:
        ctx.run('docker logout')

    exit_invoke(0, 'The container {} was promoted to {}', container, promoted_container)
